...
class Amp_Column extends Element_Base {
	...
	public function get_title() {
		return __( 'Amp Column', 'elementor' );
	}
	...
	public function amp_elementor_widget_styles(){
		...
		$color_text = '';
		if( isset( $settings['color_text']) && !empty($settings['color_text'])){
			$color_text = 'color:'.$settings['color_text'].';';
		}
		$background_overlay_color = '';
		if( isset($settings['background_overlay_color'])){
			$background_overlay_color = 'background-color:'.$settings['background_overlay_color'].';';
		}
		$background_overlay_opacity = '';
		if( !empty($settings['background_overlay_opacity']['unit']) && !empty($settings['background_overlay_opacity']['size'])){
		...
		$dynamicStyles = '.elementor-element-'.$this->get_id().' > .elementor-element-populated{
			'.$background_color.''.$color_text.''.$padding.''.$background_image.''.$background_position.''.$background_size.''.$margin.'
		}';
		$overlay = $background_overlay_color.''.$background_overlay_opacity;
		if( $overlay != '' ){
			$BgOpacityStyles = '.elementor-element-'.$this->get_id().' > .elementor-element-populated > .elementor-background-overlay{
				'.$background_overlay_color.''.$background_overlay_opacity.'
			}'; 
		}
		$amp_elemetor_custom_css['amp-column'][$this->get_id()] = $inline_styles.''.$dynamicStyles.''.$BgOpacityStyles;
	}
	public function before_render() {
		$settings = $this->get_settings_for_display();
		$this->amp_elementor_widget_styles();
		/* if( $this->get_id() == 'c1yhurs'){
			print_r($settings);
			die;
		} */
